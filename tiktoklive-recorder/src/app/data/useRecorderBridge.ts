...
export function useRecorderBridge() {
  ...
  useEffect(() => {
    ...
    const applyBridgeData = (payload: BridgePayload) => {
      if (!active) {
        return;
      }
    ...
    const scheduleReconnect = () => {
      ...
      const delay = Math.min(WS_MAX_BACKOFF_MS, WS_INITIAL_BACKOFF_MS * 2 ** reconnectAttempt);
      reconnectAttempt += 1;
    ...
    const handleRealtimeSnapshot = (snapshot: RealtimeSnapshotPayload) => {
      ...
      reconnectAttempt = 0;
      stopPolling();
      applyBridgeData(snapshot.bridgePayload);
    };
