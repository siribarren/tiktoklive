...
from lead_classifier import classify_comment, normalize_message_for_scoring, normalize_text
...
def build_tiktok_client(unique_id: str) -> TikTokLiveClient:
    web_timeout_seconds = _float_env("TIKTOK_WEB_TIMEOUT_SECONDS", 25)
    connect_timeout_seconds = _float_env("TIKTOK_CONNECT_TIMEOUT_SECONDS", 25)
    proxy_url = os.getenv("TIKTOK_PROXY_URL", "").strip()
    web_proxy = None
    ws_proxy = None
    if proxy_url:
        try:
            web_proxy = httpx.Proxy(proxy_url)
            ws_proxy = web_proxy
        except Exception as exc:
            print(f"Proxy inválido en TIKTOK_PROXY_URL: {exc}")
    ...
    tt_target_idc = os.getenv("TIKTOK_TT_TARGET_IDC", "").strip()
    if session_id and tt_target_idc:
        try:
            client.web.set_session(session_id=session_id, tt_target_idc=tt_target_idc)
        except Exception as exc:
            print(f"No se pudo aplicar TIKTOK_SESSION_ID al cliente web: {exc}")
    elif session_id and not tt_target_idc:
        print("TIKTOK_SESSION_ID definido sin TIKTOK_TT_TARGET_IDC; se omite sesión autenticada.")
...
class LeadCandidate:
    ...
    scored_message_keys: List[str] = field(default_factory=list)
    status: str = "new"
    ...
    def to_dict(self) -> Dict[str, object]:
        return {
            ...
            "scored_message_keys": self.scored_message_keys,
            "status": self.status,
        }
...
class LeadStore:
    ...
    def update(self, record: Dict[str, object]) -> None:
        ...
        with self._lock:
            ...
            normalized_message_key = normalize_message_for_scoring(str(record["comment_text"]))
            has_been_scored = bool(
                normalized_message_key and normalized_message_key in lead.scored_message_keys
            )
            if not has_been_scored:
                lead.lead_score += int(record["lead_score"])
                if normalized_message_key:
                    lead.scored_message_keys.append(normalized_message_key)
                    lead.scored_message_keys[:] = lead.scored_message_keys[-500:]
...
class TikTokCommentRecorder:
    def __init__(
        ...
        self.client = build_tiktok_client(unique_id=unique_id)
    ...
    def stop(self, timeout: float = 5.0) -> None:
        ...
        future = asyncio.run_coroutine_threadsafe(stop_client(), loop)
        try:
            ...
            print(
                f"No se pudo detener @{self.client.unique_id} antes de {timeout:.0f}s; "
                "el cierre continuara en segundo plano."
            )
        except Exception as exc:
            print(f"No se pudo detener @{self.client.unique_id}: {exc}")
        finally:
            self._finalize_session()
    ...
    def run(self):
        ...
        try:
            self._connect_task = self._loop.create_task(self.client.connect())
            return self._loop.run_until_complete(self._connect_task)
