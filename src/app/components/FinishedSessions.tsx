...
export function FinishedSessions() {
  ...
  const deleteEndedSession = async (session: (typeof filteredEndedSessions)[number]) => {
    ...
    try {
      const response = await fetch('/recorder-api/sessions', {
        method: 'DELETE',
        headers: {
                  ...
                  return (
                    ...
                    >
                      <td className="py-4 px-4">
                        <p className="text-sm font-medium text-gray-900">{session.accountName}</p>
                      </td>
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          <p className="text-xs font-mono text-gray-600 break-all">{session.id}</p>
                          <Button
                            type="button"
                            variant="ghost"
                            size="sm"
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Link
                            to={`/messages?sessionId=${encodeURIComponent(
                              session.rawSessionId ?? session.id
